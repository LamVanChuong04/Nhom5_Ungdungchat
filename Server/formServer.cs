    ...
    public partial class Server : Form
    {
        ...
        private void privateChat(List<string> text)
        {
            try
            {
                ...
                TcpClient workerSocket = (TcpClient)clientList.FirstOrDefault(x => x.Key == text[1]).Value; // Tìm client theo username trong dictionary
                Console.WriteLine("Sending data to: " + text[1]);
                ...
                if (workerSocket != null)
                {
                    ...
                    {
                        MessageBox.Show("Error writing to client: " + ex.Message);
                    }

                }
                else
                {
                    MessageBox.Show("Client not found: " + text[1]);
                }
            }
            catch (SocketException se)
            {
        ...
        private void inputPrivate_TextChanged(object sender, EventArgs e)
        {
